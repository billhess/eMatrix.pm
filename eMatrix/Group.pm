...
# NAME:  new() 
#
# DESC:  Creates new eMatrix Group Object
#
# ARGS:  Attribute hash:
#        -name       str
#        -desc       str
...
# NAME:  get_children() 
#
# DESC:  Gets a list of Group objects based on -child
#
# ARGS:  NONE
#
...
# NAME:  list_group() 
#
# DESC:  Gets a list of Group objects
#        First check if the objects have already been created
#        using the Group name as the unique ID - if not then make a call 
...
sub list_group {
   ...
   if($domql) {
      if($mdb->{-mql}) {
         my %r = $mdb->{-mql}->execute(qq(list group "$name" select *));
      ...
      if(! $rc) {
         ...
         foreach (@output) {	 
            if($_ =~ /^group /i) {
               push @list, eMatrix::Group->new(%hash) if ! $first;
               $first = 0;
         ...
         push @list, eMatrix::Group->new(%hash) if $hash{-name} ne "";
      }
   }
